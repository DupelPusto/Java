...
public class Gameplay {
    ...
    public static void playGame(Scanner sc, char[][] board) {
        ...
        while (true) {
            ...
            String player = (counter % 2 != 0) ? Constants.nameX : Constants.name0;
            ...
            while (true) {
                if (sc.hasNextInt()) {
                    ...
                    if (pos1 >= 1 && pos1 <= Constants.pzsize && pos2 >= 1 && pos2 <= Constants.pzsize) {
            ...
            if (checkWin(board, currentpl)) {
                ...
                Constants.winner = currentpl;
                Constants.winnername = player;
                Statistic.writestatistic();
                break;
            }

            if (counter == Constants.pzsize * Constants.pzsize) {
                System.out.println("Нічия!");
                Constants.draw = "Нічия";
                Statistic.writestatistic();
                break;
            }
            counter++;
    ...
    public static boolean checkWin(char[][] board, char player) {
        ...
        for (int i = 0; i < Constants.pzsize; i++) {
            for (int j = 0; j <= Constants.pzsize - winlength; j++) {
        ...
        for (int j = 0; j < Constants.pzsize; j++) {
            for (int i = 0; i <= Constants.pzsize - winlength; i++) {
                if (board[2 * i + 2][2 * j + 2] == player &&
        ...
        while (true) {
            ...
            try {
                ...
                if (set == 1){
                    ...
                    Constants.nameX = sc.nextLine();
                    System.out.println("Введіть ім'я для гравця 0: ");
                    Constants.name0 = sc.nextLine();
                    return;
                } else if (set == 0) {
                    return;
                }
